...
export function MalfuzatClient({
  ...
  const [fetchedDetail, setFetchedDetail] = useState<{ id: string; data: MalfuzatDetail | null } | null>(null)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    fetchMalfuzatDetail(effectiveSelectedId).then(d => {
      if (cancelled) return
  ...
  const selectItem = (id: string) => {
    setSelectedId(id)
    if (!isDesktop) router.push(`/malfuzat/${id}`)
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:items-stretch lg:flex-1 lg:min-h-0">
      ...
      <div className="hidden lg:flex print:flex lg:flex-1 lg:min-h-0 print:w-full print:h-auto">
        <MalfuzatDetailPanel detail={detail} loading={detailLoading} hasSelection={!!effectiveSelectedId} />
      </div>
    </div>
  )
}
...
function MalfuzatRow({ item, selected, onSelect }: {
  ...
  return (
    <button
      onClick={event => {
        if (event.ctrlKey || event.metaKey) { openDetailInNewTab(`/malfuzat/${item.id}`); return }
        onSelect()
      }}
      className={cn(
...
function MalfuzatDetailPanel({ detail, loading, hasSelection }: {
  detail: MalfuzatDetail | null
  loading: boolean
  ...
  return (
    <div className="flex flex-col w-full min-h-0 rounded-2xl border border-border bg-card overflow-hidden print:overflow-visible print:border-none print:rounded-none">
      {!hasSelection ? (
        ...
        <>
          <AdminEditButton entity="malfuzat" id={detail.id} />
          <div className="shrink-0 flex items-start justify-between gap-4 p-6 pb-4 border-b border-border/60">
            ...
            <div className="flex items-center gap-1 shrink-0 print:hidden">
              ...
              <ShareActions
                url={`${typeof window !== 'undefined' ? window.location.origin : ''}/malfuzat/${detail.id}`}
                title={detail.title}
...
function AudioDetail({ detail, audioUrl }: { detail: MalfuzatDetail; audioUrl: string }) {
  const t = useTranslations('MalfuzatPage')
...
function TextDetail({ detail }: { detail: MalfuzatDetail }) {
  const t = useTranslations('MalfuzatPage')
  return detail.body ? (
    <>
      <div
    ...
    <p className="text-base text-muted-foreground">{detail.excerpt ?? t('detailNotFound')}</p>
  )
}
