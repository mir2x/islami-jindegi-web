...
async function fetchMasails(opts: {
...
}): Promise<{ data: MasailListItem[]; total: number }> {
  ...
  try {
    const res = await fetch(`${BASE}/api/masail?${q}`)
    if (!res.ok) return { data: [], total: 0 }
    const r: PagedResult<MasailListItem> = await res.json()
    return { data: r.data, total: r.total }
  } catch { return { data: [], total: 0 } }
}

async function fetchMasailDetail(id: string): Promise<MasailDetail | null> {
  try {
    const res = await fetch(`${BASE}/api/masail/${id}`)
...
interface Props {
  ...
  initialTotal: number
  categories: MasailCategoryOption[]
  authors: MasailAuthorOption[]
...
function MasailRow({ item, selected, onSelect }: {
  ...
  return (
    <button
      onClick={event => {
        if (event.ctrlKey || event.metaKey) { openDetailInNewTab(`/masail/${item.id}`); return }
        onSelect()
      }}
      className={cn(
