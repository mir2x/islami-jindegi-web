...
export default async function BookReaderPage({
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      >
        <ArrowLeft className="w-4 h-4" /> সব কিতাব
      </Link>
      ...
      <div className="flex flex-col sm:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          {book.publisher && (
            <p className="text-sm text-muted-foreground mb-1">প্রকাশক: {book.publisher}</p>
          )}
          {book.price && (
            <p className="text-sm text-muted-foreground mb-4">মূল্য: {book.price}</p>
          )}
          {book.excerpt && (
            <div
