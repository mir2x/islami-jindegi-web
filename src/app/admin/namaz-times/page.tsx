...
export default function NamazTimesPage() {
  ...
  const [deleting, setDeleting] = useState<NamazTimeListItem | null>(null)
    ...
    <div className="h-full flex flex-col overflow-hidden">
      <div className="shrink-0 px-8 pt-8 pb-4 bg-background">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Namaz Times</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {result ? <><span className="font-semibold text-foreground">{result.total.toLocaleString()}</span> entries</> : 'Loading...'}
            </p>
          </div>
          <Button onClick={() => router.push('/admin/namaz-times/new')} className="gap-2 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-56 max-w-sm">
            ...
            <Input placeholder="Search..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} className="pl-9 bg-card" />
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm">
              <tr className="border-b">
                ...
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Bangla Title</th>
            ...
            <tbody className="divide-y divide-border/60">
              ...
              {!loading && result?.data.length === 0 && (
                <tr><td colSpan={4} className="px-5 py-20 text-center">
                  <div className="inline-flex w-14 h-14 rounded-2xl bg-muted items-center justify-center mb-4"><Clock className="w-6 h-6 text-muted-foreground/60" /></div>
                  <p className="font-medium">No namaz times found</p>
      ...
      <Dialog open={!!deleting} onOpenChange={o => !o && setDeleting(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete namaz time?</DialogTitle>
