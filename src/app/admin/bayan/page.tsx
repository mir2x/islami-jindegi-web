    ...
    <div className="h-full flex flex-col overflow-hidden">
      <div className="shrink-0 px-8 pt-8 pb-4 bg-background">
        <div className="flex items-start justify-between mb-6">
          ...
          <Button onClick={() => router.push('/admin/bayan/new')} className="gap-2 shadow-sm">
          ...
          <table className="w-full">
            <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm rounded-t-xl">
              <tr className="border-b">
                ...
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Location</th>
            ...
            <tbody className="divide-y divide-border/60">
              ...
              {!loading && result?.data.map((item: BayanListItem) => (
                <tr key={item.id} className="hover:bg-muted/30 transition-colors group cursor-pointer" onClick={() => router.push(`/admin/bayan/${item.id}/edit`)}>
                  ...
                  <td className="px-5 py-4"><span className="text-sm text-muted-foreground">{item.location ?? '—'}</span></td>
                  <td className="px-5 py-4"><span className="text-sm text-muted-foreground">{new Date(item.publishedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span></td>
                  <td className="px-5 py-4">
                    {item.published
