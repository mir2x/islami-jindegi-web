    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex gap-8 mb-8">
        ...
        <div className="shrink-0">
          {book.coverUrl ? (
            <img
              src={book.coverUrl}
              alt={book.title}
              className="w-40 rounded-xl shadow-md border object-cover"
            />
          ) : (
        ...
        <div className="flex-1 min-w-0 pt-1">
          <div className="flex items-start gap-3 mb-2">
            ...
            {book.published ? (
              <span className="mt-1 shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-0.5">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                Published
              </span>
            ) : (
              <span className="mt-1 shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-2.5 py-0.5">
      ...
      <div>
        ...
        {book.chapters.length === 0 ? (
          ...
          <ChapterList chapters={book.chapters} expandAll={expandAll} />
        )}
      </div>
...
function ChapterList({ chapters, expandAll }: { chapters: Chapter[]; expandAll: boolean }) {
  return (
    <div className="space-y-2">
      {chapters.map((chapter, i) => (
        <ChapterRowControlled key={chapter.id} chapter={chapter} index={i + 1} forceExpanded={expandAll} />
      ))}
    </div>
  )
}
