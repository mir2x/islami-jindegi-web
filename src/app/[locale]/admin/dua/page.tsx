...
import { Plus, Search, Pencil, Trash2, Sparkles, Check, ChevronsUpDown, X } from 'lucide-react'
...
type SortKey = 'position' | 'title' | 'categories' | 'language' | 'published'
import type { DuaListItem } from '@/types'

const PAGE_SIZE = 20

export default function DuaPage() {
  const navigate = useAdminNavigation()
  const { result, loading, fetch, remove, setOfflineAvailable, lastParams, setLastParams } = useDuaStore()
  ...
  const [deleting, setDeleting] = useState<DuaListItem | null>(null)
  ...
  }, [fetch, page, search, categoryId, published, offlineOnly, sortParam])
    ...
    <div className="min-h-full flex flex-col">
      <div className="shrink-0 px-4 sm:px-8 pt-8 pb-4 bg-background">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Dua</h1>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-56 max-w-sm">
            ...
            <Input placeholder="Search dua..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} className="pl-9 bg-card" />
          </div>
          ...
          <table className="w-full min-w-[1060px] table-fixed">
            ...
            <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm rounded-t-xl">
              <tr className="border-b">
                ...
                <SortableHeader label="Categories" sortKey="categories" sort={sort} onSort={handleSort} />
            ...
            <tbody className="divide-y divide-border/60">
              ...
              {!loading && result?.data.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-20 text-center">
                  <div className="inline-flex w-14 h-14 rounded-2xl bg-muted items-center justify-center mb-4"><Sparkles className="w-6 h-6 text-muted-foreground/60" /></div>
                  <p className="font-medium">No dua found</p>
              ...
              {!loading && result?.data.map((item: DuaListItem) => (
                <tr key={item.id} className="hover:bg-muted/30 transition-colors group cursor-pointer" onClick={e => navigate(`/admin/dua/${item.id}/edit`, e)}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-wrap gap-1">
                      {item.categories.slice(0, 2).map(c => <Badge key={c.id} variant="outline" className="text-xs">{c.title}</Badge>)}
                      {item.categories.length > 2 && <span className="text-xs text-muted-foreground">+{item.categories.length - 2}</span>}
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1 justify-end opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" onClick={e => { e.stopPropagation(); navigate(`/admin/dua/${item.id}/edit`, e) }} className="h-8 w-8 text-muted-foreground hover:text-foreground"><Pencil className="w-3.5 h-3.5" /></Button>
      ...
      <Dialog open={!!deleting} onOpenChange={o => !o && setDeleting(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete dua?</DialogTitle>
