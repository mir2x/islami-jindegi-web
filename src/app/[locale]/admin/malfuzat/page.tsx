...
type SortKey = 'position' | 'title' | 'author' | 'language' | 'audio' | 'published'
import type { MalfuzatListItem } from '@/types'

const PAGE_SIZE = 20

export default function MalfuzatPage() {
  const navigate = useAdminNavigation()
  const { result, loading, fetch, remove, setOfflineAvailable, lastParams, setLastParams } = useMalfuzatStore()
  ...
  const [deleting, setDeleting] = useState<MalfuzatListItem | null>(null)
    ...
    <div className="min-h-full flex flex-col">
      <div className="shrink-0 px-4 sm:px-8 pt-8 pb-4 bg-background">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-6">
          ...
          <Button nativeButton={false} render={<Link href="/admin/malfuzat/new" />} className="gap-2 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-56 max-w-sm">
            ...
            <Input placeholder="Search malfuzat..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} className="pl-9 bg-card" />
          </div>
          ...
          <table className="w-full min-w-[1140px] table-fixed">
            ...
            <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm rounded-t-xl">
              <tr className="border-b">
                ...
                <SortableHeader label="Audio" sortKey="audio" sort={sort} onSort={handleSort} />
            ...
            <tbody className="divide-y divide-border/60">
              {loading && Array.from({ length: PAGE_SIZE }).map((_, i) => (
                <tr key={i}>
                  ...
                  <td className="px-5 py-4"><Skeleton className="h-5 w-10 rounded-full" /></td>
              ...
              {!loading && result?.data.length === 0 && (
                <tr><td colSpan={8} className="px-5 py-20 text-center">
                  <div className="inline-flex w-14 h-14 rounded-2xl bg-muted items-center justify-center mb-4"><ScrollText className="w-6 h-6 text-muted-foreground/60" /></div>
                  <p className="font-medium">No malfuzat found</p>
                  <p className="text-sm text-muted-foreground mt-1">{search ? 'Try a different search' : 'Add your first entry'}</p>
                </td></tr>
              )}
              {!loading && result?.data.map((item: MalfuzatListItem) => (
                <tr key={item.id} className="hover:bg-muted/30 transition-colors group cursor-pointer" onClick={e => navigate(`/admin/malfuzat/${item.id}/edit`, e)}>
                  ...
                  <td className="px-5 py-4">
                    {item.hasAudio
                      ? <span className="inline-flex items-center gap-1 text-xs font-medium text-blue-400 bg-blue-500/10 border border-blue-500/20 rounded-full px-2 py-0.5">Audio</span>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1 justify-end opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" onClick={e => { e.stopPropagation(); navigate(`/admin/malfuzat/${item.id}/edit`, e) }} className="h-8 w-8 text-muted-foreground hover:text-foreground"><Pencil className="w-3.5 h-3.5" /></Button>
      ...
      <Dialog open={!!deleting} onOpenChange={o => !o && setDeleting(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete malfuzat?</DialogTitle>
