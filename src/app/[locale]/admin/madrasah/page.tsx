...
export default function MadrasahPage() {
  ...
  const { result, loading, fetch, remove, setOfflineAvailable } = useMadrasahStore()
          ...
          <table className="w-full min-w-[660px]">
            ...
            <tbody className="divide-y divide-border/60">
              ...
              {!loading && result?.data.map(item => (
                <tr key={item.id} className="hover:bg-muted/30 transition-colors group cursor-pointer" onClick={e => navigate(`/admin/madrasah/${item.id}/edit`, e)}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1 justify-end opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" onClick={e => { e.stopPropagation(); navigate(`/admin/madrasah/${item.id}/edit`, e) }} className="h-8 w-8 text-muted-foreground hover:text-foreground"><Pencil className="w-3.5 h-3.5" /></Button>
