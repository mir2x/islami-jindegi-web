...
import { CONTENT_MODULES, moduleLabel } from '@/lib/modules'
...
function ReorderCategories() {
  ...
  const { categories, loading, fetch, reorder } = useCategoryStore()

  const searchParams = useSearchParams()
  const requested = searchParams.get('module')
  const [module, setModule] = useState<string>(
    CONTENT_MODULES.some(m => m.key === requested) ? requested! : CONTENT_MODULES[0].key,
  )
  ...
  const items = useMemo(() => {
    ...
    const byId = new Map(inModule.map(c => [c.id, c]))
    return draft.map(id => byId.get(id)).filter((c): c is Category => !!c)
  }, [draft, inModule])

  function move(from: number, to: number) {
    if (to < 0 || to >= items.length || from === to) return
    const next = items.map(c => c.id)
  ...
  async function handleSave() {
    setSaving(true)
    try {
      await reorder(module, items.map(c => c.id))
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-5">
      <div className="mb-4">
        ...
        <h1 className="text-2xl font-bold">Reorder Categories</h1>
      ...
      <div className="bg-card border rounded-xl p-4 sm:p-5 space-y-4">
        <div className="flex items-end gap-3">
          <div className="space-y-1.5 flex-1 max-w-56">
            <Label>Module</Label>
            <Select value={module} onValueChange={v => setModule(v ?? CONTENT_MODULES[0].key)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground py-8 text-center">
            No categories are assigned to {moduleLabel(module)} yet.
          </p>
        ) : (
          <ul className="space-y-1.5">
            {items.map((c, i) => (
              ...
              >
                ...
                <span className="w-7 text-xs text-muted-foreground tabular-nums shrink-0">{i + 1}</span>
